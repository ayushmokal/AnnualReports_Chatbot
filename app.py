...
def get_conversational_chain():
    ...
    provided context just say, "answer is not available in the Report ask something else", don't provide the wrong answer\n\n
    Context:\n {context}?\n
    Question: \n{question}\n
    ...
    model = ChatGoogleGenerativeAI(model="gemini-1.5-pro-latest", client=genai, temperature=1)
...
def main():
    ...
    with st.sidebar:
        st.title("Menu:")
        pdf_docs = st.file_uploader("Upload your Annual Report PDF Files and Click on the Submit & Process Button", accept_multiple_files=True)
    ...
    if "messages" not in st.session_state.keys():
        st.session_state.messages = [
            {"role": "assistant", "content": "Upload some Annual Report PDFs and ask me a question"}
        ]
